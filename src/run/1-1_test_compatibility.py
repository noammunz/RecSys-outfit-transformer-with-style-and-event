...
def validation(args):
    ...
    # all_image_dict = polyvore.load_all_image_dict(args.polyvore_dir, metadata)

    test = polyvore.PolyvoreCompatibilityDataset(
        dataset_dir=args.polyvore_dir, dataset_type=args.polyvore_type, 
        dataset_split='test', metadata=metadata, all_embeddings_dict=all_embeddings_dict#, all_image_dict=all_image_dict
    )
