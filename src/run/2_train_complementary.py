...
def train_step(
    ...
    model.train()
    pbar = tqdm(dataloader, desc=f'Train Epoch {epoch+1}/{args.n_epochs}', disable=True)
    ...
    for i, data in enumerate(pbar):
        ...
        batched_c_embs = []
        n_candidates = min(args.batch_sz_per_gpu, batched_a_emb.size(0))
        for i in range(batched_a_emb.size(0)):
            batched_n_embs = [batched_a_emb[j] for j in range(n_candidates) if i != j]
            if len(batched_n_embs) == 0:  # Ensure at least one negative sample
                continue
        ...
        labels = torch.zeros(preds.size(0), dtype=torch.long, device=rank)
...
def train(
    ...
    # Logging Setup
    project_name = f'complementary_{args.polyvore_type}_' + (
